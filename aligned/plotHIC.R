...
hi_c_plot=ggplot(all_hic_data, aes(x = x, y = y, fill = log_counts)) +
  geom_tile() +
  facet_grid(chromosome2 ~ chromosome1,space = "free", scales = "free")+#, scales = "free") +  
  scale_fill_gradient2(low = "#ebf5fb", mid = "#e74c3c",   high = "black", midpoint = 10) +
  theme_void() +
  labs(fill = "Interaction (log2)") + theme(panel.spacing = unit(0, "lines"))

ggsave("hi_c_plot.pdf", hi_c_plot, width=8, height = 6)
