...
pppp <- ggplot()+
  ...
  scale_x_continuous(labels = function(x){sprintf("%.1fMbp",x*1e-6)}, 
                     sec.axis = sec_axis(trans = identity, 
                                         ...
                                         labels=fai$CHROM[1:11]))+
  theme(axis.text.x.top = element_text(vjust = 0.5))+
    #angle=90, vjust = 0.5))+
  rasterize(
    geom_ribbon(Coverage, mapping = aes(x=POS_absolute, 
      ymin = meancov-sdcov, 
      ymax = meancov+sdcov), fill="#526374ff", alpha=.8))+
    geom_line(Coverage, mapping=aes(x=POS_absolute, y=meancov), 
              color="#f8c26eff",linewidth=0.6) +
    theme_bw()+ xlab("Genomic position")+ ylab("Depth of coverage") +
...
saveRDS(pppp, snakemake@output[['Coverage']])